    ...
    public class SharedFrameworkPublisher
    {
        ...
        private void GenerateRuntimeGraph(DotNetCli dotnetCli, string destinationDeps)
        {
            ...
            if (!string.IsNullOrEmpty(runtimeGraphGeneratorRuntime))
            {
                ...
                var runtimeGraphGeneratorProject = Path.Combine(Dirs.RepoRoot, "tools-local", "setuptools", "independent", runtimeGraphGeneratorName);
                var runtimeGraphGeneratorOutput = Path.Combine(Dirs.Output, "tools-local", "setuptools", "independent", runtimeGraphGeneratorName);
