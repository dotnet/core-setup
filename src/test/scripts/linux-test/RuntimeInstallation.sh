...
install_runtime_packages_yum(){
	yum localinstall -y dotnet-runtime-deps-$runtime_version-$distro.$version-$arch.rpm
        rpm -Uvh dotnet-host-$runtime_version-$arch.rpm
        rpm -i dotnet-hostfxr-$runtime_version-$arch.rpm
        rpm -i dotnet-runtime-$runtime_version-$arch.rpm
}
