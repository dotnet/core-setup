...
namespace
{
        ...
        std::vector<pal::string_t>& probe_realpaths)
    {
        ...
        deps_file = get_last_known_arg(opts, opts_deps_file, _X(""));
    ...
    int32_t* required_buffer_size)
{
    ...
    fx_definition_vector_t fx_definitions;
    std::vector<pal::string_t> probe_realpaths;
    int rc = read_config_for_app(
        host_info,
        app_candidate,
        opts,
        mode,
        deps_file,
        additional_deps_serialized,
        fx_definitions,
        probe_realpaths);
    if (rc != StatusCode::Success)
        return rc;
    ...
    void** delegate)
{
    ...
    fx_definition_vector_t fx_definitions;
    std::vector<pal::string_t> probe_realpaths;
    int rc = read_config_for_lib(host_info, runtime_config, fx_definitions, probe_realpaths);
    if (rc != StatusCode::Success)
        return rc;
